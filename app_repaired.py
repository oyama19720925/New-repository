...
if not csv_files:
    st.error("❁ECSVファイルが見つかりません。stock_systemフォルダにCSVを置ぁEください、E)
    st.stop()

selected_csv = st.sidebar.selectbox("📂 CSVファイルを選抁E, csv_files)
...
if missing_cols:
    st.error(f"❁E忁Eな列が見つかりません: {missing_cols}")
    st.write("📋 実際の列名:", df_all.columns.tolist())
    st.stop()

# ========== サイドバーEフィルタ ==========
st.sidebar.header("🔧 フィルタ設宁E)
...
if COL_MARKET in df_all.columns:
    ...
    selected_markets = st.sidebar.multiselect("市場を選択（未選抁E全てEE, markets)
else:
    selected_markets = []
...
if COL_SECTOR in df_all.columns:
    ...
    selected_sectors = st.sidebar.multiselect("業種を選択（未選抁E全てEE, sectors)
else:
    selected_sectors = []

# ========== サイドバーEE析ツール ==========
st.sidebar.header("📊 刁EチEEル")

# --- 移動平坁E---
st.sidebar.subheader("📉 移動平坁E)
use_ma = st.sidebar.checkbox("移動平坁E使用", value=True)
if use_ma:
    ma_short = st.sidebar.number_input("短期MAE日EE, min_value=1, max_value=200, value=5)
    ma_long  = st.sidebar.number_input("長期MAE日EE, min_value=1, max_value=200, value=25)
    ma_cond  = st.sidebar.selectbox("条件", ['ゴールチEクロス', '短朁E> 長朁E, '短朁E< 長朁E])
else:
    ma_short = 5
    ma_long  = 25
    ma_cond  = 'ゴールチEクロス'
...
if use_rsi:
    rsi_period = st.sidebar.number_input("RSI期間E日EE, min_value=2, max_value=100, value=14)
...
# --- ストキャスチEクス ---
st.sidebar.subheader("📈 ストキャスチEクス")
use_stoch = st.sidebar.checkbox("ストキャスチEクスを使用", value=False)
if use_stoch:
    stoch_k   = st.sidebar.number_input("%K期間E日EE, min_value=1, max_value=100, value=14)
    stoch_d   = st.sidebar.number_input("%D期間E日EE, min_value=1, max_value=100, value=3)
    stoch_cond = st.sidebar.selectbox("条件", ['ゴールチEクロス', '%K > %D', '%K < %D'])
    stoch_pos_filter = st.sidebar.checkbox("%K の篁Eでフィルタ", value=False)
    if stoch_pos_filter:
        stoch_pos_min = st.sidebar.slider("%K 最小値EEEE, 0, 100, 20)
        stoch_pos_max = st.sidebar.slider("%K 最大値EEEE, 0, 100, 80)
    else:
        stoch_pos_min = 0
        stoch_pos_max = 100
else:
...
# --- 出来髁E---
st.sidebar.subheader("📦 出来髁E)
use_volume = st.sidebar.checkbox("出来高を使用", value=False)
if use_volume:
    vol_days = st.sidebar.number_input("平坁E数", min_value=1, max_value=60, value=5)
    vol_min  = st.sidebar.number_input("最小E来高（平坁EE, min_value=0, value=100000, step=10000)
else:
    vol_days = 5
    vol_min  = 100000
...
st.write(f"📋 対象チEEタ: {len(target_df):,} 衁E/ {target_df[COL_CODE].nunique():,} 銘柄")
...
with col1:
    run_btn = st.button("🔍 スクリーニング実衁E, type="primary", use_container_width=True)

with col2:
    run_all_btn = st.button("📋 全銘柄表示E条件なし！E, use_container_width=True)

# ========== スクリーニング実衁E==========
if run_btn:
    if not use_ma and not use_rsi and not use_stoch and not use_volume:
        st.warning("⚠EE刁EチEEルがEてOFFです。少なくとめEつをONにしてください、E)
    else:
        with st.spinner("⏳ スクリーニング中..."):
            result_df = do_screening(target_df)

        if result_df.empty:
            st.warning("⚠EE条件に合う銘柄が見つかりませんでした。条件を緩めてみてください、E)
        else:
            st.success(f"✁E{len(result_df)} 銘柄が条件に一致しましたEE)
...
# ========== チャート表示 ==========
st.header("📊 個別チャーチE)
chart_code = st.text_input("銘柄コードを入力（侁E 7203EE, value="")

if chart_code:
    chart_df = df_all[df_all[COL_CODE] == chart_code].sort_values(COL_DATE).reset_index(drop=True)

    if chart_df.empty:
        st.warning(f"⚠EE銘柄コーチE{chart_code} のチEEタが見つかりません、E)
    else:
        ...
        fig = make_subplots(
            ...
            subplot_titles=("ローソク足 + MA", "出来髁E, "RSI / ストキャスチEクス")
        )
